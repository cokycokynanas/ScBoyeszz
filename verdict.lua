...
local function setHitbox(enabled)
    ...
    if enabled and multiplier > 1 then
        ...
        for part, originalSize in pairs(originalSizeData) do
            if part.Parent == charToRestore then 
                pcall(function() 
...
local function findAimbotTarget()
    ...
    if selectedAimbotPlayerName and selectedAimbotPlayerName ~= "" then
        ...
        for _, player in ipairs(Players:GetPlayers()) do
            if player ~= LocalPlayer and player.Character and player.Character:FindFirstChild("Head") and getHumanoid() and getHumanoid().Health > 0 then
...
Main:CreateToggle({
    ...
    local AimbotPlayerDropdown = CombatTab:CreateDropdown({
        ...
        Options = {""} -- Opsi default: Target Terdekat
            .. sortedPlayerNames(),
        CurrentOption = {""},
    ...
    setConn("aimbotPlayerAdd", Players.PlayerAdded:Connect(function()
        pcall(function() 
            ...
            AimbotPlayerDropdown:Refresh(options, true) 
        end)
    end))
