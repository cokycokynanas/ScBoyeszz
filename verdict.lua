...
-- Fungsi untuk menemukan target terdekat
local function findClosestTarget()
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Character and player.Character:FindFirstChild("HumanoidRootPart") and getHumanoid() and getHumanoid().Health > 0 then
...
local function doAimbot()
    ...
    if flags.aimbotLock and not aimbotTarget then
        aimbotTarget = findClosestTarget()
    elseif not flags.aimbotLock then
        aimbotTarget = nil
    end
    ...
    if aimbotTarget and aimbotTarget.Character and aimbotTarget.Character:FindFirstChild("Head") then
        ...
        -- Lakukan perhitungan target ulang jika target terlalu jauh
        if (hrp.Position - targetPosition).Magnitude > 150 then
            aimbotTarget = nil -- Reset target jika terlalu jauh
        end
    else
        aimbotTarget = nil -- Reset jika target tidak valid
        if flags.aimbotLock then
            aimbotTarget = findClosestTarget() -- Coba cari target baru
        end
    end
end
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Character and player.Character:FindFirstChild("HumanoidRootPart") and player.Character:FindFirstChildOfClass("Humanoid") and player.Character:FindFirstChildOfClass("Humanoid").Health > 0 then
            ...
            if distance <= killAuraRange then
                -- Teleport ke target dan paksa serangan (metode tergantung tool)
                -- Ini adalah metode dasar yang mungkin tidak berfungsi di game yang terproteksi
                
                -- Simpan posisi asli (opsional, untuk teleport back)
                local originalCFrame = hrp.CFrame
                
                -- Teleport ke target (misalnya 1 unit di atas target)
                hrp.CFrame = targetHRP.CFrame * CFrame.new(0, 1, 0)
                
                -- Panggil remotes atau fungsi serangan (asumsi menggunakan Tool standar)
                if tool:FindFirstChild("Handle") then
                    ...
                    -- Coba panggil remotes jika diketahui (hanya contoh, tidak akan berfungsi universal)
                    -- ReplicatedStorage:WaitForChild("DamageRemote"):FireServer(player.Character) 
                end
                
                -- Teleport kembali (opsional)
...
Main:CreateToggle({
    ...
    -- Simpan data ukuran asli (HumanoidRootPart/Parts)
    local originalSizeData = {}

    local function setHitbox(multiplier)
        hitboxMultipler = multiplier
        local char = getCharacter()
        if not char then return end
            ...
            setConn("hitboxCharAdded", LocalPlayer.CharacterAdded:Connect(function()
                ...
                if newChar then
                    for _, part in ipairs(newChar:GetChildren()) do
                        if part:IsA("BasePart") then
                            part.Size = part.Size * hitboxMultipler
                        end
                    end
                end
            end))
