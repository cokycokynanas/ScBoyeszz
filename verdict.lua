...
local flags = {
    -- >> PERBAIKAN 1: Inisialisasi flags global untuk ESP dan Infinite Jump <<
    playerESP = false,      -- Default ESP OFF
    infiniteJump = true     -- Default Infinite Jump ON (AUTO-ON)
} 
...
flags.showESPHead = true -- Flag baru untuk Head Marker
...
local function createOrUpdateBox(player, hrp)
    ...
    if not head then box.Visible = false return end -- Tambahkan cek untuk head

    local headPos, headIn = worldToScreenPoint(head.Position + Vector3.new(0, 0.5, 0)) -- Sedikit offset ke atas
    local hrpPos, hrpIn = worldToScreenPoint(hrp.Position - Vector3.new(0, 0.5, 0)) -- Sedikit offset ke bawah
    
    -- >> PERBAIKAN 3: Logika Box Drawing disesuaikan agar lebih stabil <<
    if headIn and hrpIn then
        ...
        box.Position = UDim2.new(0, headPos.X, 0, top + (height / 2)) -- Tengah box
...
local function createOrUpdateText(player, hrp, hum)
    ...
    local headMarker = espTable[player].HeadMarker
    ...
    -- Update Head Marker Position and Visibility (Menggunakan flags.showESPHead)
    local head = player.Character:FindFirstChild("Head")
    if head and headMarker then
        headMarker.CFrame = head.CFrame
        headMarker.Visible = flags.playerESP and flags.showESPHead
    end
    ...
    local healthBar = billboard:FindFirstChild("InfoContainer"):FindFirstChild("HealthBar")
    ...
    -- 2. Health
    if healthBar and healthFill and hum and hum.MaxHealth > 0 then
        ...
        healthBar.Visible = flags.playerESP and flags.showESPHealth
    end
    ...
    billboard.Visible = flags.playerESP and (flags.showESPNameDist or flags.showESPHealth)
end
...
local function updateESP()
    ...
    for _, player in ipairs(Players:GetPlayers()) do
        if player ~= LocalPlayer then
            ...
            -- >> PERBAIKAN 4: Cek flags.playerESP dipindahkan ke sini untuk clean-up saat OFF <<
            if char and hrp and hum then 
                if flags.playerESP then -- Hanya jalankan drawing jika ESP ON
                    if not espTable[player] then
                        espTable[player] = {}
                    end
                    
                    -- Pastikan Adornee diperbarui pada HRP baru (setelah respawn)
                    ...
                    -- Gunakan titik tengah layar untuk startPos Line
                    local startLinePos = Vector2.new(screenCenter.X, Camera.ViewportSize.Y) -- Dari bawah tengah layar
                    
                    if hrpIn and flags.showESPLine then
                        -- Hanya update line jika pemain terlihat atau line diizinkan
                        createOrUpdateLine(player, startLinePos, hrpPos)
                    else
                        -- Hapus line jika tidak terlihat dan flags.showESPLine ON (optional, tapi baik)
                        if espTable[player].Line then espTable[player].Line.Visible = false end
                    end

                    -- 3. Box (2D)
                    createOrUpdateBox(player, hrp)
                else
                    -- ESP is OFF, clean up for this player
                    for _, data in pairs(espTable[player] or {}) do
                        if data and (data.Parent == ScreenGui or data.Parent == LocalPlayer.PlayerGui or data.Parent == Workspace) then
                            data:Destroy()
                        end
                    end
                    espTable[player] = nil
                ...
                -- Player is dead or Character missing, clean up
                for _, data in pairs(espTable[player] or {}) do
                    if data and (data.Parent == ScreenGui or data.Parent == LocalPlayer.PlayerGui or data.Parent == Workspace) then
                        data:Destroy()
                    end
...
-- ===============================================
--          INITIAL LOGIC & UI INIT
-- ===============================================

-- >> PERBAIKAN 2: Koneksi Awal untuk Infinite Jump (Auto-On) <<
...
Main:CreateToggle({
    ...
    -- Infinite Jump (Perbaikan: CurrentValue diatur ke flags.infiniteJump)
    Main:CreateToggle({
        Name = "Infinite Jump",
        CurrentValue = flags.infiniteJump,
        Callback = function(enabled)
    ...
    local function AddESPTab(Window)
        ...
        ESPTab:CreateToggle({
            ...
            CurrentValue = flags.playerESP, -- Menggunakan flags.playerESP (default false)
            Callback = function(enabled)
                ...
                if enabled then
                    ...
                    -- Menggunakan RenderStepped untuk sinkronisasi kamera yang lebih baik
                    ...
                    for player, data in pairs(espTable) do
                        for _, visual in pairs(data) do
                            if visual and (visual.Parent == ScreenGui or visual.Parent == LocalPlayer.PlayerGui or visual.Parent == Workspace) then
                                visual:Destroy()
                            end
                        end
                    end
        ...
        ESPTab:CreateToggle({
            Name = "ESP Head (Wajah)",
            CurrentValue = true,
            Callback = function(enabled) flags.showESPHead = enabled end -- Flag terpisah
        })
        ...
        ESPTab:CreateButton({
            ...
            Callback = function()
                ...
                if flags.playerESP then 
                    notify("ESP di-refresh dan diaktifkan kembali.")
                    -- Panggil ulang updateESP sekali untuk memastikan visual muncul jika ESP ON
