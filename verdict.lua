Baik, ini adalah full code skrip Lua Anda yang telah disisipkan fitur Player ESP ke dalam fungsi CreateUI().
Saya telah memastikan bahwa fitur Save & Teleport Pos (yang merupakan fitur yang Anda tanyakan sebelumnya) sudah ada dan berfungsi di bawah tab Teleport. Fitur Player ESP yang baru ditambahkan berada di tab ESP.
...
Main:CreateToggle({
    ...
    TeleportTab:CreateButton({ Name = "Clear All Slots", Callback = function()
        for i=1,12 do savedSlots[i] = nil end
                    ...
                    if char and char:FindFirstChild("HumanoidRootPart") and flags.playerESP then
                        ...
                        if hrp and espTable[player] and espTable[player].Billboard then
                            ...
                            if textLabel then
                                ...
                                if hrpLocal then
                                    local distance = (hrpLocal.Position - hrp.Position).Magnitude
