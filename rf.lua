...
local settingsTable = {
	General = {
		...
		rayfieldOpen = {Type = 'bind', Value = 'K', Name = 'BangBoyeszz Tools Keybind'},
		-- buildwarnings
		-- rayfieldprompts
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			...
			if useStudio then
				file = [[
		{"General":{"rayfieldOpen":{"Value":"K","Type":"bind","Name":"BangBoyeszz Tools Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"BangBoyeszz Tools Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
	...
	if not success then 
		if writefile then
			warn('BangBoyeszz Tools had an issue accessing configuration saving capability.')
		end
	end
end
...
local Rayfield = useStudio and script.Parent:FindFirstChild('Boyeszz') or game:GetObjects("rbxassetid://10804731440")[1]
...
repeat
	...
	if not warned then
		warn('BangBoyeszz Tools | Build Mismatch')
		print('Boyeszz may encounter issues as you are running an incompatible interface version ('.. ((Rayfield:FindFirstChild('Build') and Rayfield.Build.Value) or 'No Build') ..').\n\nThis version of BangBoyeszz Tools is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, Rayfield = Rayfield, useStudio and script.Parent:FindFirstChild('Boyeszz') or game:GetObjects("rbxassetid://10804731440")[1]
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("BangBoyeszz Tools | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("BangBoyeszz Tools | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('BangBoyeszz Tools had an issue decoding the configuration file, please try delete the file and reopen BangBoyeszz Tools.') return end
	...
	for FlagName, Flag in pairs(RayfieldLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("BangBoyeszz Tools | Unable to find '"..FlagName.. "' in the save file.")
...
function RayfieldLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('BangBoyeszz Tools | Not seeing your actions in notifications?')
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('BangBoyeszz Tools Settings', 0, true)

	if TabList['BangBoyeszz Tools Settings'] then
		TabList['BangBoyeszz Tools Settings'].LayoutOrder = 1000
	end

	if Elements['BangBoyeszz Tools Settings'] then
		Elements['BangBoyeszz Tools Settings'].LayoutOrder = 1000
	end
...
function RayfieldLibrary:CreateWindow(Settings)
	...
	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				RayfieldLibrary:Notify({Title = 'Build Mismatch', Content = 'BangBoyeszz Tools may encounter issues as you are running an incompatible interface version ('.. ((Rayfield:FindFirstChild('Build') and Rayfield.Build.Value) or 'No Build') ..').\n\nThis version of BangBoyeszz Tools is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
		...
		task.spawn(function()
			while true do
				...
				RayfieldLibrary:Notify({
					Title = "BangBoyeszz Tools Interface",
	...
	if Settings.Discord and Settings.Discord.Enabled and not useStudio then
		...
		if isfile and not isfile(RayfieldFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(RayfieldFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"BangBoyeszz Tools RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("BangBoyeszz Tools | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with BangBoyeszz Tools specific development.')
				end
			end
		end
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("BangBoyeszz Tools | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with BangBoyeszz Tools specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("BangBoyeszz Tools | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with BangBoyeszz Tools specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("BangBoyeszz Tools | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with BangBoyeszz Tools specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("BangBoyeszz Tools | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with BangBoyeszz Tools specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("BangBoyeszz Tools | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with BangBoyeszz Tools specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("BangBoyeszz Tools | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with BangBoyeszz Tools specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
	...
	if not success then warn('BangBoyeszz Tools had an issue creating settings.') end
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['BangBoyeszz Tools Settings'])
		end)
	end)
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				RayfieldLibrary:Notify({Title = "BangBoyeszz Tools Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			RayfieldLibrary:Notify({Title = "BangBoyeszz Tools Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('BangBoyeszz Tools Configurations Error | '..tostring(result))
			RayfieldLibrary:Notify({Title = "BangBoyeszz Tools Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	--	Name = "BangsNgek Tools Example Window",
	--	LoadingTitle = "BangsNgek Tools Interface Suite",
	--	Theme = 'Default',
	--	Icon = 0,
	...
	--		FileName = "Key", -- It is recommended to use something unique as other scripts using BangsNgek Tools may overwrite your key file
	--		SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
	--		GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like BangsNgek Tools to get the key from
	...
	----RayfieldLibrary:Notify({Title = "BangsNgek Tools Interface", Content = "Welcome to Rayfield. These - are the brand new notification design for Rayfield, with custom sizing and BangsNgek Tools calculated wait times.", Image = 4483362458})
